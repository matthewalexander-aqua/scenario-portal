...
function populateTerms() {
    ...
    for (let i = 3; i <= max; i += 3) {
        let o = document.createElement("option"); o.value = i; o.text = i + " Months";
...
function runPolicyCheck() {
    ...
    if (pc.length >= 3) { pcStatus.innerText = `Location: ${zone}`; pcStatus.style.color = (zone === "Metro") ? "#0f8f66" : "#d9534f"; }

    let propError = ""; let loanError = "";
    if (asset === "Vacant Land" && zone === "Non-Metro") { propError = "INELIGIBLE PROPERTY: Non-Metro Vacant Land is not eligible."; } 
    else if (land === "Large") { propError = "INELIGIBLE PROPERTY: Land size > 5HA is not eligible."; }
    else if (pc.length === 4 && zone === "Invalid") { propError = "INVALID POSTCODE: Enter a valid 4-digit postcode."; }

    if (!propError && loan > 0 && val > 0) {
        if (interestType === "Capitalised" && lvr > 70) { loanError = "POLICY ALERT: Max LVR is 70.00% for Fully Capitalised scenarios."; } 
        else if (asset === "Residential" || asset === "Townhouse") {
            let maxLVR = (loan > 5000000) ? 70 : 75;
            if (land === "Medium") { maxLVR = (zone === "Metro") ? 60 : 55; if (loan > 3000000) loanError = "POLICY ALERT: Loan capped at $3M for 1HA-5HA land."; }
            if (!loanError && lvr > maxLVR) loanError = `POLICY ALERT: Max LVR is ${maxLVR}% for this asset.`;
        } 
        else if (asset === "Unit") { if (loan > 3000000 || lvr > 75) loanError = "POLICY ALERT: Max $3M loan / 75% LVR for Units."; } 
        ...
        else if (asset === "Vacant Land") { if (loan > 3000000 || lvr > 60) loanError = "POLICY ALERT: Max $3M / 60% LVR for Metro Vacant Land."; }
    }

    if (propError) { propFeedback.innerHTML = `⚠️ ${propError}`; propFeedback.style.display = "block"; propFeedback.style.background = "#f8d7da"; propFeedback.style.color = "#721c24"; } 
    else if (pc.length === 4) { propFeedback.innerHTML = "✅ Property eligible per location policy."; propFeedback.style.display = "block"; propFeedback.style.background = "#d4edda"; propFeedback.style.color = "#155724"; } 
    else { propFeedback.style.display = "none"; }

    if (loanError) { loanFeedback.innerHTML = `⚠️ ${loanError}`; loanFeedback.style.display = "block"; loanFeedback.style.background = "#f8d7da"; loanFeedback.style.color = "#721c24"; } 
    else if (loan > 0 && val > 0) { loanFeedback.innerHTML = "✅ Loan figures meet standard LVR policy."; loanFeedback.style.display = "block"; loanFeedback.style.background = "#d4edda"; loanFeedback.style.color = "#155724"; } 
...
function submitScenario() {
    ...
    .then(() => {
        ...
        document.getElementById("propertyFeedback").style.display = "none"; document.getElementById("loanFeedback").style.display = "none"; document.getElementById("lvr").innerText = "0%";
